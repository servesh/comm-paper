...
\begin{abstract}
%\boldmath
Stream applications are often limited in their performance by their underlying communication system. In a typical implementation, the Operating System (OS) handles the majority of network operations. In such cases, the communication stack, which was not designed to handle such tremendous amounts of data, acts as a bottleneck and restricts the performance of the application. Therefore, a framework for computing and communicating in parallel is essential for improving performance.

We propose a high-level programming framework that integrates the packet processing operations with stream applications, and provides a common interface to execute both application and packet processing operations in parallel on multicore processors.

We utilize the ability of the network interface controller (NIC) to classify packets onto multiple hardware queues. This is combined with system software that allows direct access to these queues. We develop an interface in our framework for application processes to read or write in parallel. Our system integrates this with the application by a series of packet processing operations that transform the data from the application to packets suitable for transmission. Representing these tasks as stages in the application, we are able to parallelize them using techniques similar to those used for parallelizing regular computations.

All the communication operations in the framework are performed in userspace, bypassing the OS, to send and receive data directly over the network interface. By overcoming the OS bottleneck, and by parallelizing computations and packet operations together, we are able to boost the performance of stream applications substantially.
